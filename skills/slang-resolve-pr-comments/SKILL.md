...
- `gh pr view "$PR" --json mergeStateStatus` does not report `DIRTY` (actual merge conflicts). A status of `BEHIND` (branch is behind base but no conflicts) is acceptable — GitHub auto-merge handles it.
